...
**Nudge the parameters.** Each parameter moves a little in the direction that lowers the loss. How little is the **learning rate**. The plainest rule would be: new value = old value − learning rate × gradient. The routine that does the nudging is called the **optimizer**, and ours uses a better-behaved rule called **AdamW**. It keeps two running averages for every parameter: of its recent gradients, and of their squares. A running average forgets slowly. The first remembers about the last 10 steps and the second about the last 100. AdamW steps along the averaged gradient, divided by the typical size of that parameter's gradients. The effect is that a parameter's pace no longer depends on whether its gradients are naturally large or tiny. No parameter moves by much more than the learning rate in one step. One whose recent gradients agree with each other moves at close to the full rate. One whose gradients keep changing sign hardly moves. And one noisy batch cannot yank anything far. The "W" is the **weight decay** from part 4: each step also shrinks every matrix by a hair, so that a number has to keep earning its size.
...
**The climb at the start is the warm-up.** A fresh model is easy to improve, and every batch agrees about how. That is the danger. AdamW divides away the size of the gradient, so a parameter whose recent gradients all agree moves by the full learning rate at every step. At the very first step the two averages hold one gradient each, the division comes to exactly plus or minus one, and *every* parameter moves by the full rate. The starting weights are around 0.02, so a few steps of 0.001 in every parameter at once is a big shove. The model overshoots, the next gradient points back the way it came, and the loss jumps. In the first version of our benchmark, which used the full rate from the first step, the loss fell to 3.4 and then jumped to 7.2 at step 5, with the clip switched on. (The build log, entry 13, has it. Where it jumps, and how far, depends on the seed.) So the rate climbs from almost nothing to its peak of 0.001 over the first 100 steps.
...
A pass is not "every token once". Windows land at random, so in one pass about 37% of the text is never read at all, and about 26% is read more than once. (The 37% is 1/e, which turns up whenever you throw as many darts as there are squares.) Over 17 passes the chance that any given token was never read is about one in 24 million.
...
**The baseline: 1.7535 bits per character, with a standard deviation of 0.0054 over three seeds** (measured; the two runs of seed 1 are averaged first). The six-letter table scores 2.231. (A fourth seed joins below, for a reason of its own. With it the mean is 1.7533 and the standard deviation 0.0044, and that is what `docs/results-char-34.json` records.)

**The twins.** Same seed, same code, same machine, two hours apart: 1.7490 and 1.7517. They differ by 0.0027, and they did not even have their best moment at the same stop. Part 4 found the cause (one operation on this GPU adds numbers up in a varying order) and measured it over 300 steps. This is what it grows into over 10,000: from the middle of a run the twins are up to 0.015 apart at single stops, in both directions.

**Seed 0, three times, and a scare.** The pilot run in "How long should it train?" used seed 0 and the first version of the code, and scored 1.7404. That is lower than every run in the table above. A lucky seed, or had my rewrite made training worse? I ran seed 0 again with the rewritten code: 1.7525, in line with the table. Then the first version once more: 1.7423. Two runs of the old code, both about 0.010 better than the new code with the same seed. That looks like a verdict against the rewrite.

It is not, and the reason is worth the detour. On the CPU, where arithmetic is repeatable, the two versions of the code give *bit-identical* weights from the same seed: I trained a small model with each and compared every number. They do the same sums. On the GPU, all three seed-0 runs have the same training loss over the second half of the run, to four decimals: 1.0915, 1.0913, 1.0916. What differs is only the validation score, by an amount of the same size as the twins' wobble at a single stop. Two coin tosses came up heads.

**So one run is not a measurement.** Put the five same-seed runs together (three of seed 0, two of seed 1) and the spread *within* a seed is 0.0054. The spread *between* seeds 1, 2 and 3 is also 0.0054. On this machine the seed adds nothing we can see on top of the GPU's own last digits: a run is a draw from a bag, whatever its seed. All seven 32-bit runs of this setting together: mean 1.7494, standard deviation 0.0065, from 1.7404 to 1.7597.

That changes how to read the pilots. The 17-pass run (1.7706) is behind all seven 34-pass runs, by 0.011 to 0.030. So "longer than 17" stands, against a threshold of 0.01, though with less room than 0.030 had seemed to promise. The 68-pass run (1.7577) beats one of the seven, by 0.002. So "68 is no better than 34" stands too, and "68 lost 0.017" was mostly the luck of one 34-pass run. The rule picks 34 either way, because it asks for a gain of 0.01 before it will double. But both pilots were single draws from a bag 0.006 wide, and I had been reading their third decimal.
...
Most of the spread in the headline number is in the speaker labels: 6% of the characters, and how well a run happens to guess names it has never met. The twins differ by 0.005 on ordinary text and by 0.125 on the labels. The lucky pilot owes 0.011 of its 0.013 lead over the rewritten seed-0 run, at the last step, to the labels. (It is not quite as tidy at the last step of every run, where ordinary text spreads from 1.711 to 1.717 and the labels from 2.41 to 2.55: the labels are then the bigger half of the noise, not nearly all of it.) Seed 2 is the "worst" seed mainly because it is the most surprised by Romeo. This is why part 3 decided to report the two kinds of line separately when tokenizers are compared, and it will matter there: the deciding number is the first row, as written down in advance, and most of its noise comes from a twentieth of the text.

**The best moments** fell at 72%, 90%, 90% and 97% of the way through, and every run ended 0.006 to 0.010 worse than its best. So by 34 passes the over-fitting has just begun, which is where the rule was meant to put us.

**16-bit.** The rule said: allow it for the tokenizer comparison only if the 16-bit run lands no further from the twins' mean than the larger of the twins' own difference (0.0027) and the spread between seeds (0.0054). It landed 0.0089 away. So the comparison in part 6 will run in 32-bit, at 36 minutes a run and not 22. To be fair to 16-bit: its 1.7592 is inside the range of the 32-bit runs, and the seed-0 scare shows that the twins' 0.0027 was a flattering yardstick. So the validation score is no evidence that 16-bit learns worse. A quieter number is. Over the second half of the run the training loss of the seven 32-bit runs lies between 1.0899 and 1.0927. The 16-bit run's is 1.0952, above all seven. A small cost, about 0.003 nats, but on the one number here that does not wobble. The rule was written down first, and it says 32-bit.
...
uv run python scripts/train.py --name my-run --passes 34 --seed 1   # about 40 minutes on an M5 Max
...
Five parts in, GP-Thee has finally learned something. On two plays it has never read it scores 1.75 bits per character, give or take 0.005 from one run to the next, where a table of six-letter counts scores 2.23 and a blind guess 6.60. About 0.08 of that 1.75 is names it could not have known: that is what the 858 labels with unknown names cost beyond what the 915 known ones do.

The limits, in one place. The recipe is someone else's, tuned for another file, and we changed nothing. The length of a run was chosen from single runs, each a draw from a bag 0.006 wide, on the same two plays that do the judging. Those two plays differ from each other by 0.10, eighteen times the spread between our runs, so the test works may well score a tenth of a bit away. And this GPU does not repeat its last digits, which turned out to matter as much as the seed does.
...
Before we believed the number 1.75, it went through an independent evaluator, three rounds of planted faults, a second implementation of every baseline, seven runs where one had seemed enough, and a review of this post that caught its author quoting a sample from the wrong run. The six lines were the easy part.
