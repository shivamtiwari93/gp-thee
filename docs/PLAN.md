...
The repo is `gp-thee`. Each released checkpoint carries its parameter count, for example `GP-Thee-10M`. The count is fixed by the architecture, so it is known before training; see the build log.
