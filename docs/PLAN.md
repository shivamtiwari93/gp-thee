...
| 9. Evaluate | bits per character = total loss in nats over every target, divided by ln 2 and by the set's character count, so tokenizers are comparable. Reported per work, and separately for speaker-label lines (unseen names dominate them). Baselines, fitted on the training works and scored on validation: character n-grams of orders 1 to 8 with interpolated Witten-Bell smoothing (no constant to tune; the best is the 6-gram at 2.231), bzip2 and xz alone and after reading the training works. **Done, validation and test** (entry 20). On validation the model beats every baseline by 0.46+; on the held-out works by 0.52 (1.7958 against the 6-gram's 2.318). | the model beats the n-grams and the compressors — **met on both sets** |
| 10. Memorisation | share of 50-character windows copied verbatim from training text, **plus the whole run-length curve, the poet's words counted apart from the editor's furniture, an exhaustive greedy scan with a confirmation stage, and a leave-one-out innocent baseline over the 39 training works (BUILD_LOG entry 17; the validation plays are circular, having been chosen by this very test)** | reported per checkpoint and temperature |
| 11. Sample | **Done (entry 17):** `src/gp_thee/sampling.py`, `scripts/sample.py`, and a "speak as a character" dialogue wrapper. Prompts pass through a normaliser outside the tokenizer: straight quotes to curly, tabs and carriage returns handled, trailing spaces stripped (a space belongs to the NEXT token), anything else refused. Never prepend START to an ordinary prompt | fixed 10-prompt suite regenerated per checkpoint |
