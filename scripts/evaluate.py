...
met. "ROMEO." is a name it must spell out from nothing, thirty times a scene. So part of the validation
...
for label, part in rows:
    print(f"{label:<36}{part['characters']:>12,}{part['tokens']:>10,}{part['bits_per_character']:>20.4f}")
...
if baselines.exists():
    ...
    for label, result in json.loads(baselines.read_text(encoding="utf-8"))["results"].items():
        best_case = min(v for k, v in result.items() if k in ("bits_per_character", "after_reading_the_training_works"))
        print(f"  {label:<28}{best_case:>8.3f}")
    print(f"  {'this model':<28}{parts['all']['bits_per_character']:>8.3f}")

out = {"run": args.run, "checkpoint": args.which, "step": saved["step"], "tokenizer": name, "git_commit_of_training": saved["git_commit"], **parts}
